...
# ---------------------------------------------------------
# BASE DE DATOS LOCAL PARA TRACKING DE YIELD & CLV
# ---------------------------------------------------------
def init_db():
...
# INTERFAZ Y PESTAÑAS
# ---------------------------------------------------------
st.title("⚽ AI Football Predictor — Sistema Absoluto v4.0")
st.caption("Integración de xG Real, Alineaciones Confirmadas, Lesiones y Gestor de Yield")
...
with tab_scanner:
    ...
    api_key_football = st.text_input("2. API Key de API-Football (Para xG Real)", type="password")
    min_odds_val = st.number_input("Cuota Mínima Aceptable", value=1.25, step=0.05)
            ...
            try:
                ...
                if res.status_code == 200:
                    ...
                    for m in matches_data:
                        ...
                        # Inferencia/Consulta de xG Real
                        ...
                        picks = []
                        if p_h >= 0.80 and o_h >= min_odds_val:
                            stake = calculate_kelly_stake(p_h, o_h) * 100
                            picks.append(f"Gana {home_p} (Prob: {p_h*100:.1f}%, Stake: {stake:.1f}%)")
                        if p_a >= 0.80 and o_a >= min_odds_val:
                            stake = calculate_kelly_stake(p_a, o_a) * 100
                            picks.append(f"Gana {away_p} (Prob: {p_a*100:.1f}%, Stake: {stake:.1f}%)")
                            
                        results.append({
                            "Partido": f"<b>{home_p} vs {away_p}</b>",
                            "Cuotas (1-X-2)": f"{o_h:.2f} | -- | {o_a:.2f}",
                            "Selección Recomendada": "<br>".join(picks) if picks else "<span style='color:gray;'>Sin valor ≥80%</span>"
                        })
...
with tab_lineups:
    ...
    st.info("Ingresa el ID del partido de API-Football para comprobar lesionados antes de realizar tu apuesta.")
...
with tab_tracker:
    ...
    with st.form("add_bet_form"):
        ...
        if submitted:
            ...
            st.success("Apuesta registrada en la base de datos local.")
